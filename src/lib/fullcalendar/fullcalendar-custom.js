...
Grid.mixin({
	...
	eventSpanToSegs: function(span, event, segSliceFunc) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			seg.eventStartMS = +seg.start; // TODO: not the best name after making spans unzoned
			seg.eventDurationMS = span.end - span.start;
		}
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		titleHtml =
			'<span class="fc-title">' +
                ...
                //( event.isShared ? '<span>' + htmlEscape(seg.event.siteName) +' - </span>' + '<span>' + htmlEscape(event.calendarName) +': </span>' + (htmlEscape(event.title || '') || '&nbsp;') : (htmlEscape(event.title || '') || '&nbsp;'))  +
            ( event.isShared ? '<span>' + htmlEscape(event.title) +' ( </span><span>' + htmlEscape(seg.event.siteName) +'- </span>' + (htmlEscape(event.calendarName|| '') || '&nbsp;')+')' : (htmlEscape(event.title || '') || '&nbsp;'))  +
			'</span>';
...
var MyView = View.extend({
	...
	renderEvents: function(events) {
		// this.renderHead();	// todo, find way to add table header
		this.grid.renderEvents(events);
	},
	...
	isEventDraggable: function(event) {
		...
		renderHead: function() {
		this.headContainerEl =
			this.el.find('.fc-list-table')
				.html(this.grid.renderHeadHtml());
		this.headRowEl = this.headContainerEl.find('.fc-row');
	},
...
Its "el" is the inner-content of the above view's scroller.
*/
var MyViewGrid = Grid.extend({

	segSelector: '.fc-list-item', // which elements accept event actions
	hasDayInteractions: false, // no day selection or day clicking
	thisWeeekStart: moment().subtract(1,'day').startOf('day'),

	// slices by day
	spanToSegs: function(span) {
		var view = this.view;
		var dayStart = view.start.clone();
		var dayEnd = view.start.clone();
		var seg;
		var segs = [];
		// todo: find better way to add 1 day without mutating object faster .
		while (dayStart < view.end) {
			dayEnd.add(1, 'day');
			if(dayStart.isAfter(this.thisWeeekStart)){
				seg = intersectRanges(span, {
					start: dayStart,
					end: dayEnd
				});
	...
	renderSegList: function(segs) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			// append a day header
			if (!dayDate || !seg.start.isSame(dayDate, 'week')) {
				dayDate = seg.start.clone().startOf('week').stripTime();
				tbodyEl.append(this.dayHeaderHtml(dayDate));
			}
	...
	dayHeaderHtml: function(dayDate) {
		...
		var mainFormat = view.opt('listDayFormat');
		var altFormat = view.opt('listDayAltFormat');
		var endWeek = dayDate.clone().endOf('week');

		return '<tr class="fc-list-heading" data-date="' + dayDate.format('YYYY-MM-DD') + '" data-date-end="' + endWeek.format('YYYY-MM-DD') + '">' +
			'<td class="' + view.widgetHeaderClass + '" colspan="6">' +
				(mainFormat ?
					view.buildGotoAnchorHtml(
						dayDate,
						{ 'class': 'fc-list-heading-main' },
						// htmlEscape(dayDate.format(mainFormat) + " - " + htmlEscape(endWeek.format(mainFormat))) // inner HTML
						"Week " + htmlEscape(htmlEscape(endWeek.isoWeek())) // inner HTML
					) :
					'') +
				(altFormat ?
					view.buildGotoAnchorHtml(
	...
	fgSegHtml: function(seg) {
		...
		// pkhruasuwan, not all day but span across multiple days, set time
		if(!seg.event.allDay){
			spanStart = seg.event.startDate.clone().year(seg.start.year()).month(seg.start.month()).day(seg.start.day());
			spanEnd = seg.event.endDate.clone().year(seg.end.year()).month(seg.end.month()).day(seg.end.day());
		}
		...
		return '<tr class="' + classes.join(' ') + '">' +
			(timeHtml ?
				'<td class="fc-list-item-time ' + view.widgetContentClass + '">' +
					// seg.start.format('MMM D') + " " + timeHtml +
					"<div>" +
						seg.start.format('MMM D') +
					"</div>" +
					"<div>" +
						seg.start.format('dddd') +
					"</div>" +
				'</td>' :
				'') +
			'<td class="fc-list-item-title ' + view.widgetContentClass + '">' +
				'<a' + (url ? ' href="' + htmlEscape(url) + '"' : '') + '>' +
					// htmlEscape(seg.event.title) +
            // ( seg.event.isShared ?
            //     '<span>' + htmlEscape(seg.event.siteName) +' - </span>' +
            //     '<span>' + htmlEscape(seg.event.calendarName) +': </span>'
            //     : '') + htmlEscape(seg.event.title) +
                htmlEscape(seg.event.title) +
                ( seg.event.isShared ?
                '<span>(' + htmlEscape(seg.event.siteName) +' - </span>' +
				...
				'</a>' +
				'<div>' +
					htmlEscape(seg.event.shortDescription) +
				'</div>' +
				'<div>' +
					( seg.event.allDay? htmlEscape(timeHtml) :  htmlEscape(this.getEventTimeText(event))) +
				'</div>' +
			'</td>' +
			'<td class="fc-list-item-title ' + view.widgetContentClass + '">' +
				'<span>' +
					htmlEscape(seg.event.location) +
				'</span>' +
			'</td>' +
			'<td class="fc-list-item-marker ' + view.widgetContentClass + '">' +
				'<span class="fa fa-circle"' +
				(bgColor ?
					' style="color:' + bgColor + ' !important "' :
					'') +
				'></span>' +
				// '<span class="fc-event-dot"' +
				// (bgColor ?
			...
			'<td class="fc-list-item-title ' + view.widgetContentClass + '">' +
				'<span>' +
					(seg.event.categoryLbl? htmlEscape(seg.event.categoryLbl) : '') +
				'</span>' +
			'</td>' +
			'<td class="fc-list-item-title ' + view.widgetContentClass + '">' +
				'<span>' +
					htmlEscape(seg.event.contact) +
				'</span>' +
			'</td>' +
		'</tr>';
	},
...
fcViews.mylist = {
	'class': MyView,
	buttonTextKey: 'mylist', // what to lookup in locale files
	defaults: {
		buttonText: 'list', // text to display for English
		listTime: true, // show the time column?
		listDayFormat: 'LL', // like "January 1, 2016"
		noEventsMessage: 'There are no events at the moment.'
	}
};
...
fcViews.mylistMonth = {
	...
	duration: { year : 2 },
	defaults: {
		listDayFormat: 'LL' // change this for header formatting
	}
};
