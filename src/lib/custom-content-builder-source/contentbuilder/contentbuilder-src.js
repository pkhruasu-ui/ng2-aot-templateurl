...
(function (jQuery) {
    ...
    jQuery.contentbuilder = function (element, options) {
        ...
        var defaults = {
            zoom: '1',
            selectable: "h1,h2,h3,h4,h5,h6,p,blockquote,ul,ol,small,.edit,td,table",
            editMode: 'default',
            onRender: function () { },
            onDrop: function () { },
        ...
        this.init = function () {
            ...
            if(this.settings.isAdmin) {
                code_tag='<div class="row-html"><i class="cb-icon-code"></i></div>';
            }
        ...
        /**** Read HTML ****/
        this.html = function (customHtml) {

            var selectable = this.settings.selectable;
            jQuery('#temp-contentbuilder').html(customHtml? customHtml : $element.html());
            ...
            // If
            // for hubx dev, add the dynamic widget name in the array below when you add a new one
            var pbRemoveList = ['tab','accordion', 'table', 'timeline-accordion'].map(function(sel){ return '[data-' + sel + '-ctrl]'});
                pbRemoveList = pbRemoveList.join(',');

            jQuery('#temp-contentbuilder').find(pbRemoveList).remove();
        ...
        this.applyBehavior = function () {
            ...
            $element.contenteditor({ fileselect: fileselect, isAdmin: isAdmin, iconselect: iconselect, editable: selectable, colors: colors, editMode: editMode, toolbar: toolbar, toolbarDisplay: toolbarDisplay, onFileSelectClick: onFileSelectClick });
            ...
            $element.find('img').each(function () {
                ...
                jQuery(this).imageembed({ hiquality: hq, imageselect: imageselect, fileselect: fileselect, imageEmbed: imageEmbed, embedOriginalChecked: embedOriginalChecked, hideEmbedOriginal: hideEmbedOriginal, onImageBrowseClick: onImageBrowseClick, onImageSettingClick: onImageSettingClick, onImageSelectClick: onImageSelectClick, onFileSelectClick: onFileSelectClick, largerImageHandler: largerImageHandler, customval: customval });
                ...
                if (jQuery(this).parents('figure').length != 0) {
                    if (jQuery(this).parents('figure').find('figcaption').css('position') == 'absolute') {
                        jQuery(this).parents('figure').imageembed({ hiquality: hq, imageselect: imageselect, fileselect: fileselect, imageEmbed: imageEmbed, embedOriginalChecked: embedOriginalChecked, hideEmbedOriginal: hideEmbedOriginal, onImageBrowseClick: onImageBrowseClick, onImageSettingClick: onImageSettingClick, onImageSelectClick: onImageSelectClick, onFileSelectClick: onFileSelectClick, largerImageHandler: largerImageHandler, customval: customval });
                    }
                }
...
(function (jQuery) {
    ...
    jQuery.contenteditor = function (element, options) {
        ...
        this.init = function () {
            ...
            if(this.settings.isAdmin){
                code_button='<a href="#" data-rte-cmd="html" title="HTML"> <i class="hub-icon-HTML"></i> </a>';
            }

            var html_rte = '<div id="rte-toolbar"' + toolbar_attr + '>' +
					    '<a href="#" data-rte-cmd="bold" title="Bold"> <i class="hub-icon-Bold"></i> </a>' +
					    '<a href="#" data-rte-cmd="italic" title="Italic"> <i class="hub-icon-Italic"></i> </a>' +
					    '<a href="#" data-rte-cmd="underline" title="Underline"> <i class="hub-icon-Underline"></i> </a>' +
					    '<a href="#" data-rte-cmd="strikethrough" title="Strikethrough"> <i class="hub-icon-Strikethrough"></i> </a>' +
                        //'<a href="#" data-rte-cmd="fontsize" title="Font Size"> <i class="hub-icon-TextStyle"></i> </a>' +
                        '<a href="#" data-rte-cmd="formatPara" title="Paragraph"> <i class="hub-icon-TextStyle"></i> </a>' +
                        '<a href="#" data-rte-cmd="color" title="Color"> <i class="hub-icon-Color"></i> </a>' +
                        '<a href="#" data-rte-cmd="align" title="Alignment"> <i class="hub-icon-Alignment"></i> </a>' +
                        '<a href="#" data-rte-cmd="list" title="List"> <i class="hub-icon-List" style="font-size:14px;line-height:1.3"></i> </a>' +
                        '<a href="#" data-rte-cmd="createLink" title="Link"> <i class="hub-icon-Link"></i> </a>' +
                        '<a href="#" data-rte-cmd="unlink" title="Remove Link"> <i class="hub-icon-Unlink"></i> </a>' +
                        '<a href="#" data-rte-cmd="removeFormat" title="Clean"> <i class="hub-icon-Clean"></i> </a>' +
                        //'<a href="#" data-rte-cmd="formatPara" title="Paragraph"> <i class="cb-icon-header"></i> </a>' +
                        //This Font symbol is removed to disable the user to change the fonts
                ...
                '<div class="md-modal" id="md-createlink">' +
			        '<div class="md-content">' +
				        '<div class="md-body">' +
                            ...
                            (bUseCustomFileSelect ? '<input type="text" id="txtLink" class="inptxt" style="float:left;width:82%;" value="http:/' + '/"></input><i class="cb-icon-link md-btnbrowse" id="btnLinkBrowse" style="width:10%;"></i>' : '<input type="text" id="txtLink" class="inptxt" value="http:/' + '/" style="float:left;width:82%"></input>') +
                            '<br style="clear:both">' +
                            '<div style="width: 100%; line-height: 30px; height: 30px; font-size: 12px;" class="md-label"><em>For an email address please use: <strong>mailto:your-email@your-domain.com</strong></em></div>' +
                            '<br style="clear:both">' +
                            '<div class="md-label">Text:</div>' +
                            '<input type="text" id="txtLinkText" class="inptxt" style="float:left;width:82%"></input>' +
                            '<br style="clear:both">' +
                            ...
                            '<br style="clear:both">' +
                            '<div class="md-label">Select Style:</div>' +
                            '<div for="chkNewWindow" class="d-inline-block pb-button-style-block">'+
                            '<label for="external-primary" class="form-check-label"><input class="form-check-input" type="radio" name="md-button-radio" id="external-primary"><button class="btn btn-primary btn-external md-buttons" id="external-primary-btn">EXTERNAL</button></label>'+
                            '<label for="external-secondary" class="form-check-label pb-external-secondary"><input class="form-check-input" type="radio" name="md-button-radio" id="external-secondary"><button class="btn btn-secondary btn-external md-buttons" id="external-secondary-btn">EXTERNAL</button></label>'+
                            '<label for="external-link" class="form-check-label pb-external-link"><input class="form-check-input" type="radio" name="md-button-radio" id="external-link"><button class="btn-link link-external md-buttons" id="external-link-btn">EXTERNAL</button></label>'+'</br>'+
                            '<label for="internal-primary" class="form-check-label"><input class="form-check-input" type="radio" name="md-button-radio" id="internal-primary"><button class="btn btn-primary btn-internal md-buttons" id="internal-primary-btn">INTERNAL</button></label>'+
                            '<label for="internal-secondary" class="form-check-label pb-internal-secondary"><input class="form-check-input" type="radio" name="md-button-radio" id="internal-secondary"><button class="btn btn-secondary btn-external md-buttons" id="internal-secondary-btn">INTERNAL</button></label>'+
                            '<label for="internal-link" class="form-check-label pb-internal-link"><input class="form-check-input" type="radio" name="md-button-radio" id="internal-link"><button class="btn-link link-internal md-buttons" id="internal-link-btn">INTERNAL</button></label>'+'</br>'+
                            '<label for="download-primary" class="form-check-label"><input class="form-check-input" type="radio" name="md-button-radio" id="download-primary"><button class="btn btn-primary btn-download md-buttons" id="download-primary-btn">DOWNLOAD</button></label>'+
                            '<label for="download-secondary" class="form-check-label"><input class="form-check-input" type="radio" name="md-button-radio" id="download-secondary"><button class="btn btn-secondary btn-download md-buttons" id="download-secondary-btn">DOWNLOAD</button></label>'+
                            '<label for="download-link" class="form-check-label "><input class="form-check-input" type="radio" name="md-button-radio" id="download-link"><button class="btn-link link-download md-buttons" id="download-link-btn">DOWNLOAD</button></label>'+
                            '</div>'+
                            '<br style="clear:both">' +
				        '</div>' +
					    '<div class="md-footer">' +
                ...
                '<div class="md-modal" id="md-align" style="background:#fff;border-radius:12px">' +
			        '<div class="md-content">' +
				        '<div class="md-body">' +
                            ...
                            '<button class="md-pickalign" data-align="left" title="Left" style="color:#000"> <i class="cb-icon-align-left"></i> <span>Left</span> </button>' +
                            '<button class="md-pickalign" data-align="center" title="Center" style="color:#000"> <i class="cb-icon-align-center"></i> <span>Center</span> </button>' +
                            '<button class="md-pickalign" data-align="right" title="Right" style="color:#000"> <i class="cb-icon-align-right"></i> <span>Right</span> </button>' +
                            '<button class="md-pickalign" data-align="justify" title="Full" style="color:#000"> <i class="cb-icon-align-justify"></i> <span>Full</span> </button>' +
                            '</div>' +
				        '</div>' +
			        '</div>' +
		        '</div>' +
                ...
                '<div class="md-modal" id="md-list" style="background:#fff;border-radius:12px">' +
			        '<div class="md-content">' +
				        '<div class="md-body">' +
                            '<div style="padding:15px 0 15px 15px;">' +
                            '<button class="md-picklist" data-list="indent" title="Indent" style="color:#000"> <i class="cb-icon-indent-left"></i> <span>Indent</span></button>' +
                            '<button class="md-picklist" data-list="outdent" title="Outdent" style="color:#000"> <i class="cb-icon-indent-right"></i><span>Outdent</span> </button>' +
                            '<button class="md-picklist" data-list="insertUnorderedList" title="Bulleted List" style="color:#000"> <i class="cb-icon-list-bullet"></i> <span>Bulleted</span> </button>' +
                            '<button class="md-picklist" data-list="pb-check-unordered-list" title="Custom List" style="color:#000"> <i class="fa fa-check-circle-o" aria-hidden="true"></i> <span>Custom</span> </button>' +
                            '<button class="md-picklist" data-list="insertOrderedList" title="Numbered List" style="color:#000"> <i class="cb-icon-list-numbered"></i> <span>Numbered</span> </button>' +
                            '<button class="md-picklist" data-list="normal" title="Remove List" style="color:#000"> <i class="cb-icon-cancel"></i> <span>Remove</span> </button>' +
                            '</div>' +
				        '</div>' +
			        '</div>' +
		        '</div>' +
                ...
                '<div class="md-modal" id="md-fontsize" style="border-radius:12px">' +
			        '<div class="md-content" style="border-radius:12px">' +
				        '<div class="md-body">' +
                            ...
                            '<button class="md-pickfontsize" data-font-size="" data-font-family="" style="display:none"></button>' +
                        '</div>' +
			        '</div>' +
		        '</div>' +
                '' +
			        ...
			        '<div class="md-content">' +
				        '<div class="md-body">' +
                            '<div style="padding:15px 0px 15px 15px;">' +
                                '<div style="width:100%">' +
                                '<select id="selColorApplyTo" style="width:85%"><option value="1">Text Color</option><option value="3">Block Background</option></select>' +
                                '<button id="btnCleanColor" style="cursor: pointer;padding: 0px;color:#000"><i class="cb-icon-eraser" style="font-size:25px"></i>Clear</button>' +
                                '</div>' +
                                '[COLORS]' +
                                '<br style="clear:both" />' +
                            '</div>' +
        ...
        this.render = function () {
            ...
            jQuery('#rte-toolbar a[data-rte-cmd="color"]').click(function (e) {
                ...
                /**** Custom Modal ****/
                jQuery('#md-color').css('max-width', '250px');
                    ...
                    if (jQuery.trim(text) != '' && jQuery(el).text() != text) {
                        ...
                        if(selColMode==1) {
                            ...
                            if(jQuery(el).html()){
                                ...
                                jQuery(el).parent().css('color', jQuery(this).css("background-color"));
                            }
                        }
                        if(selColMode==2) {
                            ...
                            if(jQuery(el).html()){
                                ...
                                jQuery(el).parent().css('background-color', jQuery(this).css("background-color"));
                            }
                        }
                    }
                    else{
            ...
            jQuery('#rte-toolbar a[data-rte-cmd="fontsize"]').click(function (e) {
                ...
                jQuery('.md-pickfontsize').click(function(){
                    ...
                    var style=jQuery(this).attr('data-font-family');
                    var weight=jQuery(this).attr('data-font-weight');

                    if (jQuery.trim(text) != '' && jQuery(el).text() != text) {
                        document.execCommand("fontSize", false, "7");
                        document.execCommand("fontFamily", false, "HelveticaNeue");
                        document.execCommand("fontWeight", false, "100");
                        var fontElements = document.getElementsByTagName("font");
                        for (var i = 0, len = fontElements.length; i < len; ++i) {
                            if (fontElements[i].size == "7") {
                                fontElements[i].removeAttribute("size");
                                fontElements[i].style.fontSize = s;
                                fontElements[i].style.fontWeight = weight;

                            }
                            fontElements[i].removeAttribute("family");
                            fontElements[i].style.fontFamily = style;
                            fontElements[i].style.fontWeight = weight;
                        }
                    }
                    else if (jQuery(el).text() == text) {//selection fully mode on text AND element. Use element then.
                        if(jQuery(el).html()){
                            ...
                            jQuery(e1).css('font-family', style);
                            jQuery(e1).css('font-weight', weight);
                        } else {
                            jQuery(el).parent().css('font-size', s);
                            jQuery(e1).parent().css('font-family', style);
                            jQuery(e1).parent().css('font-weight', weight);
                        }
                    }
                    else{
                        jQuery(el).css('font-size', s);
                        jQuery(el).css('font-family', style);
                        jQuery(el).css('font-weight', weight);
                    };
            ...
            jQuery('#rte-toolbar a[data-rte-cmd="buttons"]').unbind('click');
            jQuery('#rte-toolbar a[data-rte-cmd="buttons"]').click(function (e) {

                var savedSel = saveSelection();

                /**** Custom Modal ****/
                jQuery('#md-buttons').css('max-width', '200px');
                jQuery('#md-buttons').simplemodal();
                jQuery('#md-buttons').data('simplemodal').show();
                e.preventDefault();

                jQuery('.md-buttons').unbind('click');
                jQuery('.md-buttons').click(function(){
                    var el;
                if (window.getSelection) {//https://www.jabcreations.com/blog/javascript-parentnode-of-selected-text
                ...
                if(el.nodeName.toLowerCase()=='a'){
                    ...
                    $activeLink.attr('class', classList);
                }
                  
                jQuery('#md-buttons').data('simplemodal').hide();   
                for (var i = 0; i < instances.length; i++) {
            ...
            jQuery('#rte-toolbar a[data-rte-cmd="buttonLinks"]').unbind('click');
            jQuery('#rte-toolbar a[data-rte-cmd="buttonLinks"]').click(function (e) {

                var savedSel = saveSelection();

                /**** Custom Modal ****/
                jQuery('#md-list-buttons').css('max-width', '200px');
                jQuery('#md-list-buttons').simplemodal();
                jQuery('#md-list-buttons').data('simplemodal').show();
                e.preventDefault();
                ...
                jQuery('.md-picklist').click(function(){
                    ...
                    try {
                        if(s=='normal') {
                            ...
                            if(s == 'pb-check-unordered-list'){
                                document.execCommand('insertUnorderedList', false, null);
                                $($activeElement).addClass("pb-check-unordered-list");
                            }else{
                                document.execCommand(s, false, null);
                                $($activeElement).removeClass("pb-check-unordered-list");
                            }    
            ...
            jQuery('#rte-toolbar a[data-rte-cmd="createLink"]').click(function (e) {
                ...
                // remove selected vealues
                jQuery("input[name='md-button-radio']:checked").prop("checked", false);
                ...
                jQuery('#btnLinkOk').bind('click', function (e) {
                    ...
                    if(jQuery('#chkNewWindow').is(":checked")){
                        ...
                        $activeLink.attr('target','_self');
                    }
                    //get class name for buttin styles
                    var idVal = $("input[name='md-button-radio']:checked").attr("id");
                    var classList = $("#"+idVal+"-btn").attr("class");
                    $activeLink.attr('class',classList);
    ...
    jQuery.imageembed = function (element, options) {
        ...
        this.init = function () {
			            ...
			            '<div class="md-content">' +
				            '<div class="md-body">' +
                                ...
                                '<div id="divImgLnk">' +
                                    ...
                                    '<input type="text" id="txtAltText" class="inptxt" style="float:left;width:60%"></input>' +
                                    '<br style="clear:both">' +
                                    '<div class="md-label">Link:</div>' +
            ...
            $element.hover(function (e) {
                ...
                /* Disabling default functionality of uploadImg Icon*/
...
function cleanHTML(input) {
    ...
    var angularStripper = / (data-)?ng-[-a-zA-Z]+=(")?[\w!@#$%^&*()_+\-=\[\]{};:\\|,.<>\/? ]*(")?/g; // strip angular attributes
    var output = output.replace(angularStripper, '');
    ...
    var badAttributes = ['style', 'start','class','tabindex','ewa-click-thru','clickid'];
