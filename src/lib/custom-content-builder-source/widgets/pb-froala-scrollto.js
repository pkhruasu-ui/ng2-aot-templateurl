! function(a) {
    ...
    $.FE.PLUGINS.scrollToAnchor = function(editor) {
        ...
        function refresh($btn,tag){
            ...
            if (e.get(0) != editor.el) {
                var f = e.get(0);
                f.tagName == tag && f.hasAttribute('scroll-to') && $btn.addClass('fr-active');
            }
        }
        ...
        function toggle(){
            ...
            // editor.format.apply('a',{ 'scroll-to' : ''})
        }
    ...
    $.FE.DefineIcon("scrollToAnchor", {NAME: "bookmark"});
    $.FE.RegisterCommand("scrollToAnchor", {
        title: "scroll to anchor link",
        undo: false,
        focus: false,
        plugin: 'scrollToAnchor',
        icon: 'scrollToAnchor',
        refresh: function($btn) {
            //highlight toolbar button
            this.scrollToAnchor.refresh($btn, "A");
        },
        callback: function() {
            ...
            //
            // // show tooltip popup
            this.scrollToAnchor.toggle();
        }
    });

    // extend the default with our new button
    $.FE.DEFAULTS.linkEditButtons = $.FE.DEFAULTS.linkEditButtons.concat(["scrollToAnchor"]);
