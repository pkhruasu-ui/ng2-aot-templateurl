...
! function(a) {
    ...
    for(var i =0; i < a.FE.VIDEO_PROVIDERS.length; i++){
        if(a.FE.VIDEO_PROVIDERS[i].provider == 'vimeo'){
            // replace default vimeo regex replace with new one
            a.FE.VIDEO_PROVIDERS[i].url_regex = /(?:https?:\/\/)?(?:www\.|player\.)?vimeo.com\/(?:channels\/(?:\w+\/)?|groups\/(?:[^\/]*)\/videos\/|album\/(?:\d+)\/video\/|video\/|)(\d+)(?:[a-zA-Z0-9_\-]+)?/i;
        }
    }
