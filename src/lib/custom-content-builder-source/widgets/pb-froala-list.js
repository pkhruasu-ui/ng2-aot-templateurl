! function(a) {
    ...
    a.FE.PLUGINS.listsCustom = function(b) {
        function c(a) {
        ...
        function f(c, d) {
            e(c, d);
            for (var f = b.html.defaultTag(), g = 0; g < c.length; g++) "LI" != c[g].tagName && (f && c[g].tagName.toLowerCase() == f ? a(c[g]).replaceWith("<" + d + " class='pb-check-unordered-list'><li" + b.node.attributes(c[g]) + ">" + a(c[g]).html() + "</li></" + d + ">") : a(c[g]).wrap("<" + d + " class='pb-check-unordered-list'><li></li></" + d + ">"));
            b.clean.lists()
        }
    ...
    }, a.FE.RegisterCommand("formatULCustom", {
        title: "Unordered List Custom",
        refresh: function(a) {
            this.listsCustom.refresh(a, "UL")
        },
        callback: function() {
            this.listsCustom.format("UL")
        },
        plugin: "lists"
    }), 
    ...
    //     plugin: "lists"
    // }),
     a.FE.DefineIcon("formatULCustom", {
        NAME: "check-circle-o"
    })
